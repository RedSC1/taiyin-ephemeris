...
ChebyshevValueWithDerivative chebyshev_eval_with_first_derivative(const double* coefficients, int count, double x) {
    if (!coefficients || count <= 0) {
        ChebyshevValueWithDerivative zero = { 0.0, 0.0 };
        return zero;
    }

    // b: 切比雪夫多项式值递推变量
    // d: 一阶导数递推变量 (d_k = 2 * x * d_{k+1} - d_{k+2} + 2 * b_{k+1})
    // 一阶导数递推只依赖值递推变量，不依赖二阶导数，因此此处的
    // value/derivative 与 chebyshev_eval_with_derivative 的结果逐位一致。
    double b_k_plus_1 = 0.0;
    double b_k_plus_2 = 0.0;
    double d_k_plus_1 = 0.0;
    double d_k_plus_2 = 0.0;
    ...
    // 组合计算最终值和一阶导数
    ChebyshevValueWithDerivative result = {
        x * b_k_plus_1 - b_k_plus_2 + coefficients[0],
        x * d_k_plus_1 - d_k_plus_2 + b_k_plus_1,
    };
    return result;
