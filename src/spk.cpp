...
double precise_et_offset_seconds(
    const SplitJulianDate* jd_tdb,
    double epoch_et_seconds,
    double fallback_et_seconds
) noexcept {
    if (!jd_tdb) return fallback_et_seconds - epoch_et_seconds;
    SplitJulianDate j2000;
    SplitJulianDate epoch;
    if (!split_julian_date_from_double(JD_J2000, &j2000)
        || !add_seconds_to_split_jd(j2000, epoch_et_seconds, &epoch)) {
        return std::numeric_limits<double>::quiet_NaN();
    }
    return days_between_split_jd(epoch, *jd_tdb) * SECONDS_PER_DAY;
}

bool et_interval_contains_time(
    double start_et_seconds,
    double end_et_seconds,
    double fallback_et_seconds,
    const SplitJulianDate* jd_tdb
) noexcept {
    if (!std::isfinite(start_et_seconds) || !std::isfinite(end_et_seconds)
        || end_et_seconds < start_et_seconds) {
        return false;
    }
    if (!jd_tdb) {
        return std::isfinite(fallback_et_seconds)
            && fallback_et_seconds >= start_et_seconds
            && fallback_et_seconds <= end_et_seconds;
    }
    if (!split_julian_date_is_finite(*jd_tdb)) return false;
    SplitJulianDate start_jd;
    SplitJulianDate end_jd;
    if (!add_seconds_to_split_jd(SPLIT_JD_J2000, start_et_seconds, &start_jd)
        || !add_seconds_to_split_jd(SPLIT_JD_J2000, end_et_seconds, &end_jd)) {
        return false;
    }
...
bool eval_chebyshev_record(
    ...
    const double scaled_time = precise_et_offset_seconds(
        jd_tdb, midpoint, et_seconds) / radius;
...
bool eval_type21_record(
    ...
    const double delta = precise_et_offset_seconds(jd_tdb, tl, et_seconds);
...
bool eval_spk_segment_type21(
    ...
    if (!out || segment.spk_type != 21
        || !et_interval_contains_time(
    ...
    if (!eval_type21_record(et_seconds, jd_tdb, record, maxdim, &state)) {
        return false;
    }
    if (!compute_velocity) {
...
bool eval_direct_state(
    ...
    const SpkSegment* segment = jd_tdb
        ? find_spk_segment(kernel, target_id, center_id, *jd_tdb)
        : find_spk_segment(kernel, target_id, center_id, et_seconds);
    return segment && eval_spk_segment(
        kernel, *segment, et_seconds, jd_tdb, compute_velocity, out);
}
...
double jd_tdb_to_et_seconds(const SplitJulianDate& jd_tdb) noexcept {
    return days_between_split_jd(SPLIT_JD_J2000, jd_tdb) * SECONDS_PER_DAY;
}

double split_days_from_double(const SplitJulianDate& jd, double epoch_jd) noexcept {
    SplitJulianDate epoch;
    if (!split_julian_date_from_double(epoch_jd, &epoch)) {
        return std::numeric_limits<double>::quiet_NaN();
    }
    return days_between_split_jd(epoch, jd);
}
...
const SpkCompiledSegment* find_compiled_spk_segment(
    ...
    if (!jd_tdb && !std::isfinite(et_seconds)) {
        return 0;
    }
...
bool eval_compiled_spk_segment(
    ...
    if (!out || !et_interval_contains_time(
            segment.start_et_seconds,
            segment.end_et_seconds,
            et_seconds,
            jd_tdb)) {
    ...
    if (segment.spk_type == 21) {
        ...
        CartesianState state;
        if (!eval_type21_record(
                et_seconds, jd_tdb, record, segment.type21_maxdim, &state)) {
            return false;
        }
        if (!compute_velocity) {
            state.velocity_au_per_day.x = 0.0;
    ...
    if (segment.spk_type == 20) {
        ...
        return eval_type20_record(
            &record[0], record.size(), et_seconds, jd_tdb, directory,
            record_index, compute_velocity, out);
    }

    return eval_chebyshev_record(
        segment.spk_type, &record[0], record.size(), et_seconds, jd_tdb,
        compute_velocity, out);
}
...
bool eval_compiled_direct_state(
    ...
    const SpkCompiledSegment* segment = find_compiled_spk_segment(
        data, target_id, center_id, et_seconds, jd_tdb);
    return segment && eval_compiled_spk_segment(
        data, *segment, et_seconds, jd_tdb, compute_velocity, out);
}
...
bool eval_compiled_relative_state(
    ...
    std::vector<SpkPathStep> path;
    if (!find_compiled_spk_path(
            data, target_id, center_id, et_seconds, jd_tdb, &path)) {
        return false;
    }
...
bool compiled_spk_data_has_type21(const SpkEphemerisData& data) noexcept {
    for (size_t i = 0; i < data.segments.size(); ++i) {
        if (data.segments[i].spk_type == 21) {
            return true;
        }
    }
    return false;
...
bool finite_difference_spk_acceleration(
    ...
    const double available_before = split_days_from_double(jd_tdb, data.jd_tdb_start);
    const double available_after = -split_days_from_double(jd_tdb, data.jd_tdb_end);
    if (!out || !split_julian_date_is_finite(jd_tdb)
        || !std::isfinite(available_before) || !std::isfinite(available_after)
    ...
    if (available_before >= dt && available_after >= dt) {
        ...
        if (!add_days_to_split_jd(jd_tdb, -dt, &before_jd)
            || !add_days_to_split_jd(jd_tdb, dt, &after_jd)) {
            return false;
        }
        ...
        if (!eval_compiled_relative_state(
                ...
                data.target_id,
                data.center_id,
                jd_tdb_to_et_seconds(before_jd),
                &before_jd,
                true,
                &before)
    ...
    if (available_after > 0.0 && available_before <= available_after) {
        ...
        SplitJulianDate after_jd;
        if (!add_days_to_split_jd(jd_tdb, dt, &after_jd)) {
            return false;
        }
...
bool compile_spk_ephemeris_data_from_source(
    ...
    *out = 0;
    ...
    const bool ok = compile_spk_ephemeris_data_from_kernel(
        *kernel,
        target_id,
        center_id,
        jd_tdb_start,
...
bool compile_spk_ephemeris_data_from_kernel(
    const SpkKernel& kernel,
    int target_id,
    int center_id,
    double jd_tdb_start,
...
bool calc_spk_state_void(const SplitJulianDate& jd_tdb, const void* data, CartesianState* out) noexcept;

bool calc_spk_position_void(const SplitJulianDate& jd_tdb, const void* data, Vector3* out) noexcept {
    const SpkEphemerisData* spk_data = static_cast<const SpkEphemerisData*>(data);
    if (!out || !spk_data || !split_julian_date_is_finite(jd_tdb)
        || split_days_from_double(jd_tdb, spk_data->jd_tdb_start) < 0.0
        || split_days_from_double(jd_tdb, spk_data->jd_tdb_end) > 0.0) {
        return false;
    }
    CartesianState state;
    if (!eval_compiled_relative_state(
...
bool calc_spk_velocity_void(const SplitJulianDate& jd_tdb, const void* data, Vector3* out) noexcept {
    const SpkEphemerisData* spk_data = static_cast<const SpkEphemerisData*>(data);
    if (!out || !spk_data || !split_julian_date_is_finite(jd_tdb)
        || split_days_from_double(jd_tdb, spk_data->jd_tdb_start) < 0.0
        || split_days_from_double(jd_tdb, spk_data->jd_tdb_end) > 0.0) {
...
bool calc_spk_acceleration_void(const SplitJulianDate& jd_tdb, const void* data, Vector3* out) noexcept {
    const SpkEphemerisData* spk_data = static_cast<const SpkEphemerisData*>(data);
    if (!out || !spk_data || !split_julian_date_is_finite(jd_tdb)
        || split_days_from_double(jd_tdb, spk_data->jd_tdb_start) < 0.0
        || split_days_from_double(jd_tdb, spk_data->jd_tdb_end) > 0.0) {
    ...
    if (compiled_spk_data_has_type21(*spk_data)) {
        return finite_difference_spk_acceleration(*spk_data, jd_tdb, out);
    }
    CartesianState state;
    if (!eval_compiled_relative_state(
            *spk_data,
...
bool compile_spk_ephemeris_block_from_kernel(
    const SpkKernel& kernel,
    int target_id,
    int center_id,
    double jd_tdb_start,
    ...
    *out = StorageEphemerisBlock();
    ...
    SpkEphemerisData* data = 0;
    if (!compile_spk_ephemeris_data_from_kernel(
            kernel,
            target_id,
            center_id,
            jd_tdb_start,
...
bool calc_spk_state(const SplitJulianDate& jd_tdb, const SpkEphemerisData* data, CartesianState* out) noexcept {
    if (!data || !out || !split_julian_date_is_finite(jd_tdb)) {
        return false;
    }
    if (split_days_from_double(jd_tdb, data->jd_tdb_start) < 0.0
        || split_days_from_double(jd_tdb, data->jd_tdb_end) > 0.0) {
        return false;
    }
    if (data->target_id == data->center_id) {
        *out = CartesianState();
    ...
    if (!eval_compiled_relative_state(
            *data, data->target_id, data->center_id, et_seconds, &jd_tdb,
            true, out)) {
        return false;
    }
    if (compiled_spk_data_has_type21(*data)) {
...
const SpkSegment* find_spk_segment(
    ...
    if (!split_julian_date_is_finite(jd_tdb)) return 0;
    const double et_seconds = jd_tdb_to_et_seconds(jd_tdb);
    for (size_t i = 0; i < kernel.index.segments.size(); ++i) {
        const SpkSegment& segment = kernel.index.segments[i];
        if (segment.target_id == target_id
...
bool eval_spk_segment(
    ...
    return eval_spk_segment(
        kernel, segment, et_seconds, nullptr, compute_velocity, out);
}
...
bool eval_spk_segment(
    ...
    if (segment.spk_type == 21) {
        return eval_spk_segment_type21(
            kernel, segment, et_seconds, jd_tdb, compute_velocity, out);
    }
    if (segment.spk_type == 20) {
        SpkType20Directory directory;
        ...
        if (!et_interval_contains_time(
                directory.init_et_seconds,
                directory.init_et_seconds
                    + directory.interval_seconds * directory.record_count,
                et_seconds,
                jd_tdb)) {
            return false;
        }
...
bool eval_spk_relative_state(
    ...
    const double et_seconds = jd_tdb_to_et_seconds(jd_tdb);
    if (eval_direct_state(
            kernel, target_id, center_id, et_seconds, &jd_tdb,
            compute_velocity, out)) {
        return true;
    }
    ...
    if (eval_direct_state(
            kernel, target_id, 0, et_seconds, &jd_tdb, compute_velocity,
            &target_state)
        && eval_direct_state(
            kernel, center_id, 0, et_seconds, &jd_tdb, compute_velocity,
            &center_state)) {
