...
struct UnitVectorKinematics {
    Vector3 unit;
    Vector3 unit_rate;
    Vector3 unit_acceleration;
};

bool unit_vector_kinematics(
    const CartesianState& state,
    UnitVectorKinematics* out
) noexcept {
    if (!out) {
        return false;
    }
    ...
    const Vector3 unit = vector3_scale(state.position_au, 1.0 / distance);
    const double radial_rate = vector3_dot(unit, state.velocity_au_per_day);
    const Vector3 transverse_velocity = vector3_subtract(
        state.velocity_au_per_day,
        vector3_scale(unit, radial_rate));
    const Vector3 unit_rate = vector3_scale(transverse_velocity, 1.0 / distance);
    const double radial_acceleration =
        vector3_dot(unit_rate, state.velocity_au_per_day)
        + vector3_dot(unit, state.acceleration_au_per_day2);
    const Vector3 unit_acceleration = vector3_scale(
        vector3_subtract(
            vector3_subtract(
                state.acceleration_au_per_day2,
                vector3_scale(unit, radial_acceleration)),
            vector3_scale(unit_rate, 2.0 * radial_rate)),
        1.0 / distance);

    if (!std::isfinite(unit_rate.x)
        || !std::isfinite(unit_rate.y)
        || !std::isfinite(unit_rate.z)
        || !std::isfinite(unit_acceleration.x)
        || !std::isfinite(unit_acceleration.y)
        || !std::isfinite(unit_acceleration.z)) {
        return false;
    }

    out->unit = unit;
    out->unit_rate = unit_rate;
    out->unit_acceleration = unit_acceleration;
    return true;
}
...
bool cartesian_state_angular_separation_kinematics(
    const CartesianState& a,
    const CartesianState& b,
    AngularSeparationKinematics* out
) noexcept {
    if (!out) {
        return false;
    }

    UnitVectorKinematics ka;
    UnitVectorKinematics kb;
    if (!unit_vector_kinematics(a, &ka) || !unit_vector_kinematics(b, &kb)) {
        return false;
    }

    double cos_angle = vector3_dot(ka.unit, kb.unit);
    if (cos_angle > 1.0) {
        cos_angle = 1.0;
    } else if (cos_angle < -1.0) {
        cos_angle = -1.0;
    }
    const double separation = std::acos(cos_angle);
    const double sin_angle = std::sqrt(std::max(0.0, 1.0 - cos_angle * cos_angle));
    if (!std::isfinite(separation) || !std::isfinite(sin_angle)) {
        return false;
    }
    if (!(sin_angle > 1.0e-14)) {
        out->separation_rad = separation;
        out->separation_rate_rad_per_day = 0.0;
        out->separation_acceleration_rad_per_day2 = 0.0;
        return true;
    }

    const double cos_rate =
        vector3_dot(ka.unit_rate, kb.unit)
        + vector3_dot(ka.unit, kb.unit_rate);
    const double cos_acceleration =
        vector3_dot(ka.unit_acceleration, kb.unit)
        + 2.0 * vector3_dot(ka.unit_rate, kb.unit_rate)
        + vector3_dot(ka.unit, kb.unit_acceleration);
    const double separation_rate = -cos_rate / sin_angle;
    const double separation_acceleration =
        -cos_acceleration / sin_angle
        - cos_angle * cos_rate * cos_rate / (sin_angle * sin_angle * sin_angle);

    if (!std::isfinite(separation_rate) || !std::isfinite(separation_acceleration)) {
        return false;
    }
    out->separation_rad = separation;
    out->separation_rate_rad_per_day = separation_rate;
    out->separation_acceleration_rad_per_day2 = separation_acceleration;
    return true;
}
