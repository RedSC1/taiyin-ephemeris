...
const double TAIYIN_SOLAR_RADIUS_KM = 696000.0;
const double SOLAR_DEFLECTION_MODEL_RADIUS_EPSILON_AU = 1e-20;
...
bool is_solar_schwarzschild_radius(double schwarzschild_radius_au) noexcept {
    return std::fabs(schwarzschild_radius_au - TAIYIN_SOLAR_SCHWARZSCHILD_RADIUS_AU)
        <= SOLAR_DEFLECTION_MODEL_RADIUS_EPSILON_AU;
}
...
bool retarded_julian_date(
    const SplitJulianDate& reception_jd_tdb,
    double light_time_days,
    SplitJulianDate* out
) noexcept {
    return add_days_to_split_jd(
        reception_jd_tdb, -light_time_days, out);
}
...
bool solve_light_time_iteration(
    ...
    for (int i = 0; i < iterations; ++i) {
        SplitJulianDate emission_jd_tdb;
        if (!retarded_julian_date(jd_tdb, light_time_days, &emission_jd_tdb)
...
bool apply_gravitational_deflection_from_body_with_model(
    ...
    double denominator_limit = deflection_limit;
    double denominator_limit_dot = 0.0;
    double enclosed_mass_fraction = 1.0;
    double enclosed_mass_fraction_dot = 0.0;

    if (deflection_model_id == TAIYIN_DEFLECTION_MODEL_SOLAR_DISK
        && is_solar_schwarzschild_radius(schwarzschild_radius_au)
        && em > 0.0) {
        const double solar_radius_sin = TAIYIN_SOLAR_RADIUS_KM / (em * TAIYIN_AU_KM);
        const double solar_radius_sin_dot = -solar_radius_sin * em_dot / em;
        const double physical_limb_limit = 0.5 * solar_radius_sin * solar_radius_sin;
        const double physical_limb_limit_dot = solar_radius_sin * solar_radius_sin_dot;
        if (std::isfinite(physical_limb_limit) && physical_limb_limit > denominator_limit) {
            denominator_limit = physical_limb_limit;
            denominator_limit_dot = physical_limb_limit_dot;
        }

        if (std::isfinite(solar_radius_sin)
            && solar_radius_sin > 0.0
            && raw_denominator < 2.0 * physical_limb_limit) {
            const double impact_squared = std::max(0.0, 1.0 - p_dot_e * p_dot_e);
            const double impact_factor = std::sqrt(impact_squared);
            double impact_factor_dot = 0.0;
            if (impact_factor > 0.0) {
                impact_factor_dot = -p_dot_e * p_dot_e_dot / impact_factor;
            }
            if (impact_factor < solar_radius_sin) {
                const double x = impact_factor / solar_radius_sin;
                const double x_dot = (impact_factor_dot * solar_radius_sin
                    - impact_factor * solar_radius_sin_dot)
                    / (solar_radius_sin * solar_radius_sin);
                enclosed_mass_fraction = (1.05 * x) / (x + 0.05);
                enclosed_mass_fraction_dot = (1.05 * 0.05 * x_dot) / ((x + 0.05) * (x + 0.05));
            }
        }
    }

    const bool limited = raw_denominator < denominator_limit;
    const double denominator = limited ? denominator_limit : raw_denominator;
    const double denominator_dot = limited ? denominator_limit_dot : q_dot_e_dot;
    if (em == 0.0 || denominator == 0.0 || !std::isfinite(enclosed_mass_fraction)) {
        return false;
    }
    ...
    const double base_scale = schwarzschild_radius_au / (em * denominator);
    const double base_scale_dot = -base_scale * (em_dot / em + denominator_dot / denominator);
    const double scale = base_scale * enclosed_mass_fraction;
    const double scale_dot = base_scale_dot * enclosed_mass_fraction + base_scale * enclosed_mass_fraction_dot;
...
bool apply_gravitational_deflection_from_body(
    ...
    return apply_gravitational_deflection_from_body_with_model(
        apparent_position_au,
        apparent_velocity_au_per_day,
        observer_heliocentric_position_au,
        observer_heliocentric_velocity_au_per_day,
        ...
        deflection_limit,
        TAIYIN_DEFLECTION_MODEL_ERFA,
        out_position_au,
        out_velocity_au_per_day);
}

bool apply_gravitational_deflection_from_body_acceleration_with_model(
    const Vector3& apparent_position_au,
    const Vector3& apparent_velocity_au_per_day,
    ...
    if (!apply_gravitational_deflection_from_body_with_model(
            ...
            out_position_au,
            out_velocity_au_per_day)
        || !apply_gravitational_deflection_from_body_with_model(
...
bool apply_gravitational_deflection_from_body_acceleration(
    ...
    return apply_gravitational_deflection_from_body_acceleration_with_model(
        apparent_position_au,
        apparent_velocity_au_per_day,
        apparent_acceleration_au_per_day2,
        observer_heliocentric_position_au,
        observer_heliocentric_velocity_au_per_day,
        observer_heliocentric_acceleration_au_per_day2,
        deflector_heliocentric_position_au,
