...
bool finite_bracket(
    ...
    return std::isfinite(lower_x)
        && std::isfinite(lower_value)
        && std::isfinite(upper_x)
...
bool inside_open_interval(double value, double lower, double upper) noexcept {
    return value > lower && value < upper;
}

double clamp_step(double step, double limit) noexcept {
...
bool solve_bracketed_newton_bisection(
    ...
    double* out_last_evaluated_x,
    double* out_next_x,
    int* out_iteration_count,
    int* out_evaluation_count
) noexcept {
    if (!eval
        || !out_root_x
        || !out_value
        || !out_derivative
        || !out_last_evaluated_x
        || !out_next_x
        || !out_iteration_count
        || !out_evaluation_count
        || max_iterations <= 0
        || value_tolerance < 0.0
        || !finite_bracket(lower_x, lower_value, upper_x, upper_value)) {
        return false;
    }
    ...
    *out_value = 0.0;
    *out_derivative = 0.0;
    *out_last_evaluated_x = 0.0;
    *out_next_x = 0.0;
    *out_iteration_count = 0;
    *out_evaluation_count = 0;

    double x = inside_open_interval(initial_x, lower_x, upper_x)
        ? initial_x
        : 0.5 * (lower_x + upper_x);
    BracketedRootEval current;

    for (int iteration = 0; iteration < max_iterations; ++iteration) {
        if (!eval(x, user_data, &current)
            || !std::isfinite(current.value)
            || !std::isfinite(current.derivative)) {
            *out_next_x = x;
            return false;
        }

        *out_last_evaluated_x = x;
        *out_root_x = x;
        *out_value = current.value;
        *out_derivative = current.derivative;
        *out_iteration_count = iteration + 1;
        ++(*out_evaluation_count);

        if (std::fabs(current.value) <= value_tolerance) {
            *out_next_x = x;
            return true;
        }
        ...
        if (current.value < 0.0) {
            lower_x = x;
            lower_value = current.value;
        } else {
            upper_x = x;
            upper_value = current.value;
        }

        double next_x = 0.5 * (lower_x + upper_x);
        if (std::fabs(current.derivative) >= min_derivative) {
            const double newton_x = x - clamp_step(current.value / current.derivative, max_newton_step);
            if (std::isfinite(newton_x) && inside_open_interval(newton_x, lower_x, upper_x)) {
                next_x = newton_x;
            }
        }

        if (!std::isfinite(next_x) || next_x == x) {
            *out_next_x = next_x;
            return false;
        }
        *out_next_x = next_x;
