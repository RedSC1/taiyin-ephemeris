...
double days_from_kepler_epoch(const SplitJulianDate& jd, double epoch_jd) noexcept {
...
const KeplerElements* find_element(const SplitJulianDate& jd_tdb, const KeplerEphemerisData* data) noexcept {
    const KeplerElements* elements = data->get_elements();
    for (size_t i = 0; i < data->element_count; ++i) {
        if (days_from_kepler_epoch(jd_tdb, elements[i].jd_tdb_start) >= 0.0
            && days_from_kepler_epoch(jd_tdb, elements[i].jd_tdb_end) <= 0.0) {
            return &elements[i];
        }
    }
    return 0;
...
bool eval_kepler_orbital_basis(
    const SplitJulianDate& jd_tdb,
    const KeplerEphemerisData* data,
    const KeplerElements** out_element,
    double* out_eccentric_anomaly,
    double* out_one_minus_e_cos_e
) noexcept {
    if (!data
        || !out_element
        || !out_eccentric_anomaly
        || !out_one_minus_e_cos_e
        || !split_julian_date_is_finite(jd_tdb)
        || days_from_kepler_epoch(jd_tdb, data->jd_tdb_start) < 0.0
        || days_from_kepler_epoch(jd_tdb, data->jd_tdb_end) > 0.0) {
        return false;
    }
    ...
    const double mean_anomaly = normalize_angle(
        element->mean_anomaly_at_epoch_rad
            + n * days_from_kepler_epoch(jd_tdb, element->epoch_jd_tdb));
    ...
    const double one_minus_e_cos_e = 1.0 - e * std::cos(eccentric_anomaly);
    if (one_minus_e_cos_e <= 0.0) {
        return false;
    }

    *out_element = element;
    *out_eccentric_anomaly = eccentric_anomaly;
    *out_one_minus_e_cos_e = one_minus_e_cos_e;
    return true;
}

bool calc_kepler_position(
    const SplitJulianDate& jd_tdb,
    const KeplerEphemerisData* data,
    Vector3* out
) noexcept {
    ...
    const KeplerElements* element = 0;
    double eccentric_anomaly = 0.0;
    double one_minus_e_cos_e = 0.0;
    if (!eval_kepler_orbital_basis(jd_tdb, data, &element, &eccentric_anomaly, &one_minus_e_cos_e)) {
        return false;
    }
    (void)one_minus_e_cos_e;
    ...
    rotate_orbital_to_inertial(*element, x_orb, y_orb, out);
    return true;
}

bool calc_kepler_velocity(
    const SplitJulianDate& jd_tdb,
    const KeplerEphemerisData* data,
    Vector3* out
) noexcept {
    ...
    rotate_orbital_to_inertial(*element, vx_orb, vy_orb, out);
    return true;
}

bool calc_kepler_acceleration(
    const SplitJulianDate& jd_tdb,
    const KeplerEphemerisData* data,
    Vector3* out
) noexcept {
    ...
    Vector3 position;
    if (!calc_kepler_position(jd_tdb, data, &position)) {
        return false;
    }
    const double r2 =
    ...
    out->x = accel_scale * position.x;
    out->y = accel_scale * position.y;
    out->z = accel_scale * position.z;
    return true;
}
...
bool calc_kepler_position_void(const SplitJulianDate& jd_tdb, const void* data, Vector3* out) noexcept {
    return calc_kepler_position(jd_tdb, static_cast<const KeplerEphemerisData*>(data), out);
}

bool calc_kepler_velocity_void(const SplitJulianDate& jd_tdb, const void* data, Vector3* out) noexcept {
    return calc_kepler_velocity(jd_tdb, static_cast<const KeplerEphemerisData*>(data), out);
}

bool calc_kepler_acceleration_void(const SplitJulianDate& jd_tdb, const void* data, Vector3* out) noexcept {
    return calc_kepler_acceleration(jd_tdb, static_cast<const KeplerEphemerisData*>(data), out);
}
...
bool calc_kepler_state(
    ...
    if (!data || !out || !split_julian_date_is_finite(jd_tdb) || eccentric_anomaly_max_iterations <= 0 || eccentric_anomaly_tolerance_rad <= 0.0) {
        return false;
    }
    if (days_from_kepler_epoch(jd_tdb, data->jd_tdb_start) < 0.0
        || days_from_kepler_epoch(jd_tdb, data->jd_tdb_end) > 0.0) {
        return false;
    }
