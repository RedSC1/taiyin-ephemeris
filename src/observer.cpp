...
ObserverDerivatives true_equator_observer_derivatives_au(
    ...
    return rotating_observer_derivatives_au(
        ...
        gast_iau2000b_rad(jd_ut1, jd_tt),
        gast_rate_iau2000b_rad_per_day(jd_tt, dut1_rate_seconds_per_day, lod_seconds, OBSERVER_DERIVATIVE_STEP_DAYS),
        gast_acceleration_iau2000b_rad_per_day2(jd_tt, lod_rate_seconds_per_day, OBSERVER_DERIVATIVE_STEP_DAYS),
        xp_rad,
        yp_rad,
        sp_rad,
        xp_rate_rad_per_day,
...
Vector3 geodetic_to_ecef_m(double longitude_rad, double latitude_rad, double height_m) noexcept {
    ...
    const double sin_lon = std::sin(longitude_rad);
    const double cos_lon = std::cos(longitude_rad);
    const double n = TAIYIN_WGS84_A_M / std::sqrt(1.0 - TAIYIN_WGS84_E2 * sin_lat * sin_lat);
    const Vector3 result = {
        (n + height_m) * cos_lat * cos_lon,
        (n + height_m) * cos_lat * sin_lon,
        (n * (1.0 - TAIYIN_WGS84_E2) + height_m) * sin_lat,
    };
    return result;
}
...
Matrix3x3 terrestrial_to_true_equator_of_date_matrix(
    ...
    return matrix3x3_multiply(earth_rotation_matrix(gast_iau2000b_rad(jd_ut1, jd_tt)), polar_motion_matrix(xp_rad, yp_rad, sp_rad));
}
