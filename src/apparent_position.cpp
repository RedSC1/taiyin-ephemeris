...
struct MatrixEvalData {
    ...
    CustomOutputFrameEvaluator custom_output_frame_evaluator;
    const void* custom_output_frame_data;
};
...
bool calc_output_apparent_matrices(
    ...
    return calc_apparent_matrices(
            ...
            &dpsi_rad,
            &deps_rad,
            custom_output_frame_evaluator,
            custom_output_frame_data);
}
...
bool known_output_frame(int output_frame_id) noexcept {
    return output_frame_id == TAIYIN_APPARENT_FRAME_ICRF
        ...
        || output_frame_id == TAIYIN_APPARENT_FRAME_CIRS
        || output_frame_id == TAIYIN_APPARENT_FRAME_CUSTOM;
}

bool finite_matrix_array(const double matrix[9]) noexcept {
...
bool static_output_frame(int output_frame_id) noexcept {
    return output_frame_id == TAIYIN_APPARENT_FRAME_ICRF
        || output_frame_id == TAIYIN_APPARENT_FRAME_J2000_MEAN_EQUATOR
        || output_frame_id == TAIYIN_APPARENT_FRAME_J2000_ECLIPTIC;
}

Matrix3x3 true_equator_matrix(
    const Matrix3x3& precession,
    const NutationAngles& nutation
) noexcept {
    return matrix3x3_multiply(nutation_matrix(nutation), precession);
}

Matrix3x3 true_ecliptic_matrix(
    const Matrix3x3& precession,
    const NutationAngles& nutation
) noexcept {
...
bool build_output_matrix(
    ...
    int obliquity_model_id,
    int frame_route_id,
    const SplitJulianDate& reference_jd_tt,
    double celestial_pole_offset_dx_rad,
    double celestial_pole_offset_dy_rad,
    double celestial_pole_offset_dx_rate_rad_per_day,
    double celestial_pole_offset_dy_rate_rad_per_day,
    ...
    if (!split_julian_date_is_finite(jd_tt) || !out_output || !known_output_frame(output_frame_id)) {
        return false;
    }
    if (obliquity_model_id != 0
        || !split_julian_date_is_finite(reference_jd_tt)
        || !std::isfinite(celestial_pole_offset_dx_rad)
        || !std::isfinite(celestial_pole_offset_dy_rad)
        || !std::isfinite(celestial_pole_offset_dx_rate_rad_per_day)
        || !std::isfinite(celestial_pole_offset_dy_rate_rad_per_day)) {
        return false;
    }
    ...
    *out_output = matrix3x3_identity();
    ...
    if (output_frame_id == TAIYIN_APPARENT_FRAME_CUSTOM) {
        double matrix[9] = {};
        if (!custom_output_frame_evaluator) {
            return false;
        }
        try {
            if (!custom_output_frame_evaluator(
                    jd_tt, custom_output_frame_data, matrix)) {
                return false;
            }
        } catch (...) {
            return false;
        }
        if (!finite_matrix_array(matrix)) return false;
        *out_output = matrix_from_array(matrix);
        return true;
    }

    if (output_frame_id == TAIYIN_APPARENT_FRAME_J2000_MEAN_EQUATOR
        || output_frame_id == TAIYIN_APPARENT_FRAME_J2000_ECLIPTIC) {
        Matrix3x3 j2000_precession = matrix3x3_identity();
        double j2000_mean_obliquity = 0.0;
        const SplitJulianDate j2000(2451545, 0.0);
        if (!dispatch::eval_precession(precession_model_id, j2000, 0, &j2000_precession, &j2000_mean_obliquity)) {
            return false;
        }
        *out_output = output_frame_id == TAIYIN_APPARENT_FRAME_J2000_MEAN_EQUATOR
            ? j2000_precession
            : matrix3x3_multiply(rotation_x_matrix(j2000_mean_obliquity), j2000_precession);
        if (out_precession) {
            *out_precession = j2000_precession;
        }
        if (out_nutation_angles) {
            out_nutation_angles->mean_obliquity_rad = j2000_mean_obliquity;
            out_nutation_angles->true_obliquity_rad = j2000_mean_obliquity;
        }
        return true;
    }
    ...
    if (output_frame_id == TAIYIN_APPARENT_FRAME_MEAN_EQUATOR_OF_DATE
        || output_frame_id == TAIYIN_APPARENT_FRAME_MEAN_ECLIPTIC_OF_DATE) {
        *out_output = output_frame_id == TAIYIN_APPARENT_FRAME_MEAN_EQUATOR_OF_DATE
            ? precession
            : matrix3x3_multiply(rotation_x_matrix(mean_obliquity), precession);
        if (out_precession) {
            *out_precession = precession;
        }
        if (out_nutation_angles) {
            out_nutation_angles->mean_obliquity_rad = mean_obliquity;
            out_nutation_angles->true_obliquity_rad = mean_obliquity;
        }
        return true;
    }
    ...
    if (output_frame_id == TAIYIN_APPARENT_FRAME_CIRS) {
        const double dt_days = days_between_split_jd(reference_jd_tt, jd_tt);
        const double dx_rad = celestial_pole_offset_dx_rad
            + celestial_pole_offset_dx_rate_rad_per_day * dt_days;
        ...
        if (frame_route_id == dispatch::FRAME_ROUTE_EQUINOX) {
            *out_output = cirs_equinox_matrix(jd_tt, precession, nutation);
            ...
            frame_data.yp_rad = 0.0;
            frame_data.sp_rad = 0.0;
            frame_data.dx_rad = dx_rad;
            frame_data.dy_rad = dy_rad;
            frame_data.precession_model = precession_model_id;
            frame_data.nutation_model = nutation_model_id;
            if (!dispatch::eval_frame_route(frame_route_id, jd_tt, jd_tt, &frame_data, out_output)) {
                return false;
            }
        }
        if (out_precession) {
    ...
    const Matrix3x3 output = output_frame_id == TAIYIN_APPARENT_FRAME_TRUE_EQUATOR_OF_DATE
        ...
        : true_ecliptic_matrix(precession, nutation);
...
bool eval_output_matrix(const SplitJulianDate& jd_tt, const void* data, Matrix3x3* out_matrix) noexcept {
    ...
    return build_output_matrix(
        ...
        eval_data->frame_route_id,
        eval_data->reference_jd_tt,
        eval_data->celestial_pole_offset_dx_rad,
        eval_data->celestial_pole_offset_dy_rad,
        eval_data->celestial_pole_offset_dx_rate_rad_per_day,
        eval_data->celestial_pole_offset_dy_rate_rad_per_day,
        eval_data->custom_output_frame_evaluator,
        eval_data->custom_output_frame_data,
        0,
        0,
        out_matrix,
        0);
...
bool eval_relative_position(const SplitJulianDate& jd_tdb, const void* data, Vector3* out) noexcept {
...
bool eval_relative_velocity(const SplitJulianDate& jd_tdb, const void* data, Vector3* out) noexcept {
    const RelativeToDeflectorEvalData* eval_data = static_cast<const RelativeToDeflectorEvalData*>(data);
    if (!eval_data || !out) {
        return false;
    }
...
bool eval_relative_acceleration(const SplitJulianDate& jd_tdb, const void* data, Vector3* out) noexcept {
    const RelativeToDeflectorEvalData* eval_data = static_cast<const RelativeToDeflectorEvalData*>(data);
    if (!eval_data || !out) {
        return false;
    }
...
bool eval_deflector_relative_position(const SplitJulianDate& jd_tdb, const void* data, int deflector_index, Vector3* out) noexcept {
...
bool eval_deflector_relative_velocity(const SplitJulianDate& jd_tdb, const void* data, int deflector_index, Vector3* out) noexcept {
    const MultiDeflectorRelativeEvalData* eval_data = static_cast<const MultiDeflectorRelativeEvalData*>(data);
    if (!eval_data || !out || deflector_index < 0 || deflector_index >= eval_data->deflector_count) {
        return false;
    }
...
bool eval_deflector_relative_acceleration(const SplitJulianDate& jd_tdb, const void* data, int deflector_index, Vector3* out) noexcept {
    const MultiDeflectorRelativeEvalData* eval_data = static_cast<const MultiDeflectorRelativeEvalData*>(data);
    if (!eval_data || !out || deflector_index < 0 || deflector_index >= eval_data->deflector_count) {
        return false;
    }
...
bool eval_observer_state(
    ...
    return eval_block_state_for_flags(jd_tdb, observer_block, flags, out);
}
...
bool calc_apparent_matrices(
    ...
    if (static_output_frame(output_frame_id)) {
        return true;
    }
    ...
    eval_data.frame_route_id = frame_route_id;
    eval_data.reference_jd_tt = jd_tt;
    eval_data.celestial_pole_offset_dx_rad = celestial_pole_offset_dx_rad;
    eval_data.celestial_pole_offset_dy_rad = celestial_pole_offset_dy_rad;
    eval_data.celestial_pole_offset_dx_rate_rad_per_day = celestial_pole_offset_dx_rate_rad_per_day;
    eval_data.celestial_pole_offset_dy_rate_rad_per_day = celestial_pole_offset_dy_rate_rad_per_day;
    eval_data.custom_output_frame_evaluator = custom_output_frame_evaluator;
    eval_data.custom_output_frame_data = custom_output_frame_data;
...
bool calc_apparent_with_matrix(
    const SplitJulianDate& jd_tdb,
    int target_id,
    const internal::CompiledEphemerisBlock* target_block,
    int observer_id,
    ...
    if (!split_julian_date_is_finite(jd_tdb)
    ...
    if (light_time_method_id != 0 || shapiro_delay_model_id != 0 || !valid_deflection_model_id(deflection_model_id)) {
        return false;
    }
    ...
    if (!eval_block_state_for_flags(jd_tdb, target_block, flags, &target)) {
        return false;
    }
    ...
    if ((flags & TAIYIN_APPARENT_LIGHT_TIME) != 0u) {
        if ((flags & TAIYIN_APPARENT_SHAPIRO_DELAY) != 0u) {
            ...
            if (!eval_block_state_for_flags(jd_tdb, solar_block, flags, &solar)) {
                return false;
            }
    ...
    if ((flags & TAIYIN_APPARENT_DEFLECTION) != 0u) {
        ...
        for (int i = 0; i < deflector_count; ++i) {
            ...
            if (!eval_block_state_for_flags(jd_tdb, deflector_block, flags, &deflector)) {
                return false;
            }
            ...
            if (need_acceleration(flags)) {
                if (!apply_gravitational_deflection_from_body_acceleration_with_model(
    ...
    if ((flags & TAIYIN_APPARENT_ABERRATION) != 0u) {
        const internal::CompiledEphemerisBlock* solar_block = deflector_blocks[solar_deflector_index];
        CartesianState solar;
        if (target_id == deflector_ids[solar_deflector_index]) {
            solar = target;
        } else {
            if (!eval_block_state_for_flags(jd_tdb, solar_block, flags, &solar)) {
                return false;
...
bool calc_apparent_batch_with_matrix(
    const SplitJulianDate& jd_tdb,
    int target_count,
    ...
    const double output_matrix_dot[9],
    const double output_matrix_ddot[9],
    double* out_geometric_pos_au,
    double* out_geometric_vel_au_per_day,
    double* out_geometric_acc_au_per_day2,
    double* out_astrometric_pos_au,
    double* out_astrometric_vel_au_per_day,
    double* out_astrometric_acc_au_per_day2,
    double* out_deflected_pos_au,
    double* out_deflected_vel_au_per_day,
    double* out_deflected_acc_au_per_day2,
    double* out_aberrated_pos_au,
    double* out_aberrated_vel_au_per_day,
    double* out_aberrated_acc_au_per_day2,
    ...
    for (int i = 0; i < target_count; ++i) {
        ...
        double* geometric_pos = out_geometric_pos_au ? out_geometric_pos_au + i * 3 : 0;
        double* geometric_vel = out_geometric_vel_au_per_day ? out_geometric_vel_au_per_day + i * 3 : 0;
        double* geometric_acc = out_geometric_acc_au_per_day2 ? out_geometric_acc_au_per_day2 + i * 3 : 0;
        double* astrometric_pos = out_astrometric_pos_au ? out_astrometric_pos_au + i * 3 : 0;
        double* astrometric_vel = out_astrometric_vel_au_per_day ? out_astrometric_vel_au_per_day + i * 3 : 0;
        double* astrometric_acc = out_astrometric_acc_au_per_day2 ? out_astrometric_acc_au_per_day2 + i * 3 : 0;
        double* deflected_pos = out_deflected_pos_au ? out_deflected_pos_au + i * 3 : 0;
        double* deflected_vel = out_deflected_vel_au_per_day ? out_deflected_vel_au_per_day + i * 3 : 0;
        double* deflected_acc = out_deflected_acc_au_per_day2 ? out_deflected_acc_au_per_day2 + i * 3 : 0;
        double* aberrated_pos = out_aberrated_pos_au ? out_aberrated_pos_au + i * 3 : 0;
        double* aberrated_vel = out_aberrated_vel_au_per_day ? out_aberrated_vel_au_per_day + i * 3 : 0;
        double* aberrated_acc = out_aberrated_acc_au_per_day2 ? out_aberrated_acc_au_per_day2 + i * 3 : 0;
...
bool calc_apparent_batch(
    ...
    if (output_frame_id != TAIYIN_APPARENT_FRAME_ICRF) {
        if (!calc_output_apparent_matrices(
                jd_tt,
                flags,
                output_frame_id,
                precession_model_id,
    ...
    return calc_apparent_batch_with_matrix(
        jd_tdb,
        target_count,
        target_ids,
        target_blocks,
...
bool calc_apparent(
    ...
    return calc_apparent_with_matrix(
        jd_tdb,
        target_id,
        target_block,
        observer_id,
