...
Tsf1StarProvider::Tsf1StarProvider(size_t cache_max_entries, int catalog_id) noexcept
    : owned_tsc1_bytes_(),
      delegate_(cache_max_entries, catalog_id) {}
...
bool Tsf1StarProvider::eval_position(const std::string& id_or_alias, const SplitJulianDate& jd_tdb, Vector3* out) noexcept {
    return delegate_.eval_position(id_or_alias, jd_tdb, out);
}

bool Tsf1StarProvider::eval_velocity(const std::string& id_or_alias, const SplitJulianDate& jd_tdb, Vector3* out) noexcept {
    return delegate_.eval_velocity(id_or_alias, jd_tdb, out);
}

bool Tsf1StarProvider::eval_acceleration(const std::string& id_or_alias, const SplitJulianDate& jd_tdb, Vector3* out) noexcept {
    return delegate_.eval_acceleration(id_or_alias, jd_tdb, out);
}

bool Tsf1StarProvider::eval_state(const std::string& id_or_alias, const SplitJulianDate& jd_tdb, CartesianState* out) noexcept {
