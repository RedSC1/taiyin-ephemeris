...
enum Tsc1CacheEvalKind {
    Tsc1CacheEvalPosition,
    Tsc1CacheEvalVelocity,
    Tsc1CacheEvalAcceleration,
    Tsc1CacheEvalState
};

struct Tsc1CacheEvalContext {
    int runtime_id;
    SplitJulianDate jd_tdb;
    Tsc1CacheEvalKind kind;
    Vector3* vector_out;
    CartesianState* state_out;
};

void destroy_tsc1_storage_cache_data(void* data) noexcept {
...
bool eval_tsc1_storage_cache_data(const void* data, void* user) noexcept {
    const internal::StorageEphemerisBlock* storage =
        static_cast<const internal::StorageEphemerisBlock*>(data);
    Tsc1CacheEvalContext* context = static_cast<Tsc1CacheEvalContext*>(user);
    if (!storage || !context) {
        return false;
    }

    internal::CompiledEphemerisBlock block;
    if (!internal::get_compiled_block_from_storage(storage, context->runtime_id, &block)) {
        return false;
    }

    switch (context->kind) {
        case Tsc1CacheEvalPosition:
            return internal::eval_compiled_ephemeris_block_position(
                context->jd_tdb,
                &block,
                context->vector_out);
        case Tsc1CacheEvalVelocity:
            return internal::eval_compiled_ephemeris_block_velocity(
                context->jd_tdb,
                &block,
                context->vector_out);
        case Tsc1CacheEvalAcceleration:
            return internal::eval_compiled_ephemeris_block_acceleration(
                context->jd_tdb,
                &block,
                context->vector_out);
        case Tsc1CacheEvalState:
            return internal::eval_compiled_ephemeris_block(
                context->jd_tdb,
                &block,
                context->state_out);
    }
...
Tsc1ResolvedStar::Tsc1ResolvedStar()
    : star_index(0), runtime_id(0), record(0) {}
...
bool calc_tsc1_star_position(const SplitJulianDate& jd_tdb, const void* data, Vector3* out_position_au) noexcept {
    if (!data || !out_position_au) {
        return false;
    }
    const Tsc1StarEphemerisData* star = static_cast<const Tsc1StarEphemerisData*>(data);
    SplitJulianDate reference_jd;
    if (!split_julian_date_from_double(star->reference_jd_tdb, &reference_jd)) {
        return false;
    }
    const double dt_days = days_between_split_jd(reference_jd, jd_tdb);
    *out_position_au = vector3_add(star->position_ref_au, vector3_scale(star->velocity_ref_au_per_day, dt_days));
    return true;
}

bool calc_tsc1_star_velocity(const SplitJulianDate& jd_tdb, const void* data, Vector3* out_velocity_au_per_day) noexcept {
    (void)jd_tdb;
...
bool calc_tsc1_star_acceleration(const SplitJulianDate& jd_tdb, const void* data, Vector3* out_acceleration_au_per_day2) noexcept {
    (void)jd_tdb;
    (void)data;
...
Tsc1StarProvider::Tsc1StarProvider(size_t cache_max_entries, int catalog_id) noexcept
    : catalog_(),
      cache_(cache_max_entries),
      catalog_id_(catalog_id),
      jd_tdb_start_(TSC1_DEFAULT_JD_TDB_START),
      jd_tdb_end_(TSC1_DEFAULT_JD_TDB_END),
      runtime_id_by_index_() {}
...
bool Tsc1StarProvider::resolve(const std::string& id_or_alias, Tsc1ResolvedStar* out) noexcept {
    ...
    const int runtime_id = resolve_or_register_runtime_id(star_index, record);
    if (runtime_id == 0) {
        return false;
    }
...
bool Tsc1StarProvider::eval_position(const std::string& id_or_alias, const SplitJulianDate& jd_tdb, Vector3* out) noexcept {
    if (!out) {
        return false;
    }
    ...
    const internal::EphemerisSegmentCacheKey key = cache_key(resolved.runtime_id);
    Tsc1CacheEvalContext context = { resolved.runtime_id, jd_tdb, Tsc1CacheEvalPosition, out, 0 };
    if (cache_.with_data(key, eval_tsc1_storage_cache_data, &context)) {
        return true;
    }
    return ensure_cached(resolved.star_index, resolved.runtime_id)
        && cache_.with_data(key, eval_tsc1_storage_cache_data, &context);
}

bool Tsc1StarProvider::eval_velocity(const std::string& id_or_alias, const SplitJulianDate& jd_tdb, Vector3* out) noexcept {
    if (!out) {
        return false;
    }
    Tsc1ResolvedStar resolved;
    ...
    const internal::EphemerisSegmentCacheKey key = cache_key(resolved.runtime_id);
    Tsc1CacheEvalContext context = { resolved.runtime_id, jd_tdb, Tsc1CacheEvalVelocity, out, 0 };
    if (cache_.with_data(key, eval_tsc1_storage_cache_data, &context)) {
        return true;
    }
    return ensure_cached(resolved.star_index, resolved.runtime_id)
...
bool Tsc1StarProvider::eval_acceleration(const std::string& id_or_alias, const SplitJulianDate& jd_tdb, Vector3* out) noexcept {
    if (!out) {
        return false;
    }
    Tsc1ResolvedStar resolved;
    ...
    const internal::EphemerisSegmentCacheKey key = cache_key(resolved.runtime_id);
    Tsc1CacheEvalContext context = { resolved.runtime_id, jd_tdb, Tsc1CacheEvalAcceleration, out, 0 };
    if (cache_.with_data(key, eval_tsc1_storage_cache_data, &context)) {
        return true;
    }
    return ensure_cached(resolved.star_index, resolved.runtime_id)
...
bool Tsc1StarProvider::eval_state(const std::string& id_or_alias, const SplitJulianDate& jd_tdb, CartesianState* out) noexcept {
    if (!out) {
        return false;
    }
    Tsc1ResolvedStar resolved;
    ...
    const internal::EphemerisSegmentCacheKey key = cache_key(resolved.runtime_id);
    Tsc1CacheEvalContext context = { resolved.runtime_id, jd_tdb, Tsc1CacheEvalState, 0, out };
    if (cache_.with_data(key, eval_tsc1_storage_cache_data, &context)) {
        return true;
    }
    return ensure_cached(resolved.star_index, resolved.runtime_id)
...
bool Tsc1StarProvider::contains_cached_star(const std::string& id_or_alias) noexcept {
    ...
    return cache_.contains(cache_key(resolved.runtime_id));
}
...
bool Tsc1StarProvider::ensure_cached(uint32_t star_index, int runtime_id) noexcept {
    if (runtime_id == 0) {
        return false;
    }
    const internal::EphemerisSegmentCacheKey key = cache_key(runtime_id);
    if (cache_.contains(key)) {
        return true;
    }

    internal::StorageEphemerisBlock* storage = new (std::nothrow) internal::StorageEphemerisBlock();
    if (!storage) {
        return false;
    }
    if (!tsc1_compile_star_storage_block(&catalog_, star_index, runtime_id, storage)) {
        delete storage;
        return false;
    }

    internal::EphemerisSegmentCacheData data(storage, destroy_tsc1_storage_cache_data);
    if (!cache_.insert(key, data)) {
        destroy_tsc1_storage_cache_data(storage);
        return false;
    }
    return true;
}

int Tsc1StarProvider::resolve_or_register_runtime_id(uint32_t star_index, const Tsc1StarRecord* record) noexcept {
    std::unordered_map<uint32_t, int>::const_iterator existing = runtime_id_by_index_.find(star_index);
    if (existing != runtime_id_by_index_.end()) {
    ...
    int runtime_id = internal::register_celestial_body(canonical_id);
    const char* display_name = tsc1_catalog_string(&catalog_, record->display_name_offset);
    if (display_name && display_name[0] != '\0') {
        internal::register_celestial_body_alias(display_name, runtime_id);
    }
    ...
    for (uint32_t i = 0; catalog_.header && i < catalog_.header->alias_count; ++i) {
        ...
        if (alias_string && alias_string[0] != '\0') {
            internal::register_celestial_body_alias(alias_string, runtime_id);
        }
    }

    try {
        runtime_id_by_index_[star_index] = runtime_id;
    } catch (...) {
        return 0;
    }
    return runtime_id;
...
internal::EphemerisSegmentCacheKey Tsc1StarProvider::cache_key(int runtime_id) const noexcept {
    return internal::EphemerisSegmentCacheKey(
        internal::EphemerisSegmentCacheKindStarRecord,
        runtime_id,
        TSC1_DEFAULT_CENTER_ID,
        TSC1_STAR_METHOD_ID,
        internal::IcrfJ2000Equatorial,
        internal::EphemerisBlockKey(
            TSC1_SOURCE_ID,
            static_cast<uint64_t>(catalog_id_),
            catalog_.header ? catalog_.header->version : TSC1_VERSION,
            TSC1_PURPOSE),
        static_cast<int64_t>(runtime_id));
}
