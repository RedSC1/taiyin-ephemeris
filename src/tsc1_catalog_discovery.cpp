...
const double TSC1_DISCOVERY_J2000_JD = 2451545.0;
const double TSC1_DISCOVERY_DAYS_PER_JULIAN_YEAR = 365.25;

double epoch_year_to_jd_tdb(double epoch_year) noexcept {
    return TSC1_DISCOVERY_J2000_JD
        + (epoch_year - 2000.0) * TSC1_DISCOVERY_DAYS_PER_JULIAN_YEAR;
}
...
bool collect_tsc1_descriptors_from_directory(
    ...
    try {
        discoverers.push_back(&discover_tsc1_file);
    } catch (...) {
        return false;
    }
