...
static bool equinox(const SplitJulianDate& /*jd_ut1*/, const SplitJulianDate& jd_tt, const void* data, Matrix3x3* out) {
    ...
    *out = matrix3x3_multiply(nutation_matrix(nutation), precession);
    return true;
}

static bool cirs(const SplitJulianDate& /*jd_ut1*/, const SplitJulianDate& jd_tt, const void* data, Matrix3x3* out) {
    const FrameRouteDispatchData* d = static_cast<const FrameRouteDispatchData*>(data);
