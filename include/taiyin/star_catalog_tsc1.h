...
#include "internal/ephemeris_segment_cache.h"
...
static const uint64_t TSC1_SOURCE_ID = 3;
static const uint32_t TSC1_PURPOSE = 0;
...
public:
    Tsc1StarProvider(size_t cache_max_entries = 4096, int catalog_id = 1) noexcept;
    ...
    bool eval_position(const std::string& id_or_alias, const SplitJulianDate& jd_tdb, Vector3* out) noexcept;
    bool eval_velocity(const std::string& id_or_alias, const SplitJulianDate& jd_tdb, Vector3* out) noexcept;
    bool eval_acceleration(const std::string& id_or_alias, const SplitJulianDate& jd_tdb, Vector3* out) noexcept;
    bool eval_state(const std::string& id_or_alias, const SplitJulianDate& jd_tdb, CartesianState* out) noexcept;
...
private:
    bool ensure_cached(uint32_t star_index, int runtime_id) noexcept;
    int resolve_or_register_runtime_id(uint32_t star_index, const Tsc1StarRecord* record) noexcept;
    internal::EphemerisSegmentCacheKey cache_key(int runtime_id) const noexcept;

    Tsc1Catalog catalog_;
    internal::EphemerisSegmentCache cache_;
    int catalog_id_;
    double jd_tdb_start_;
    double jd_tdb_end_;
    std::unordered_map<uint32_t, int> runtime_id_by_index_;
};
