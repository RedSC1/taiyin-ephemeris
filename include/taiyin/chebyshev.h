...
/**
 * @brief 只存放切比雪夫多项式值和一阶导数的结构体
 *
 * 用于只需要速度、不需要加速度的求值路径。一阶导数递推不依赖二阶导数，
 * 因此该结果与 ChebyshevValue 中对应的 value/derivative 逐位一致。
 */
struct ChebyshevValueWithDerivative {
    double value;      ///< 对应 x 的多项式计算值 (通常为位置)
    double derivative; ///< 对应 x 的多项式一阶导数 (通常为速度，未做时间缩放)
};
...
/**
 * @brief 一次性计算切比雪夫多项式的值和一阶导数 (不计算二阶导数)
 *
 * 当调用方只需要速度、不需要加速度时使用，避免为二阶导数付出额外计算。
 * 一阶导数的递推不依赖二阶导数，因此返回的 value/derivative 与
 * chebyshev_eval_with_derivative 的结果逐位一致。
 *
 * @param coefficients 切比雪夫多项式系数数组的首地址
 * @param count 系数个数 (即多项式阶数 Degree + 1)
 * @param x 归一化时间参数，取值范围必须在 [-1, 1] 之间
 * @return ChebyshevValueWithDerivative 包含值和一阶导数的结构体
 */
ChebyshevValueWithDerivative chebyshev_eval_with_first_derivative(const double* coefficients, int count, double x);
