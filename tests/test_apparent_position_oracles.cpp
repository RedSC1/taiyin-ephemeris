...
void check_de441_mars_horizons_astrometric_oracle() {
    ...
    // - epoch: 2024-01-01T00:00:00Z, jd_tt below from the Horizons fixture route
    ...
    const double jd_tdb_value = tt_to_tdb_jd_fast(jd_tt);
    const double start = jd_tdb_value - 2.0;
    const double end = jd_tdb_value + 2.0;
    const taiyin::SplitJulianDate jd_tdb = split_jd(jd_tdb_value);
    ...
    expect_true(
        taiyin::calc_apparent_with_matrix(
            jd_tdb,
            4,
            &mars_block,
            399,
...
bool relative_position_from_blocks(
    const taiyin::SplitJulianDate& jd_tdb,
    const void* data,
    taiyin::Vector3* out
) {
...
bool relative_velocity_from_blocks(
    const taiyin::SplitJulianDate& jd_tdb,
    const void* data,
    taiyin::Vector3* out
) {
...
bool deflector_relative_position_from_blocks(
    const taiyin::SplitJulianDate& jd_tdb,
    const void* data,
    int index,
    taiyin::Vector3* out
...
bool deflector_relative_velocity_from_blocks(
    const taiyin::SplitJulianDate& jd_tdb,
    const void* data,
    int index,
    taiyin::Vector3* out
...
void check_de441_mars_deflection_aberration_horizons_oracle() {
    ...
    //   matching the Horizons fixture route in test_spk.cpp.
    const double jd_tt = 2460310.50080074091;
    const double jd_tdb_value = tt_to_tdb_jd_fast(jd_tt);
    const double start = jd_tdb_value - 2.0;
    const double end = jd_tdb_value + 2.0;
    ...
    expect_true(
        taiyin::calc_apparent_matrices(
            ...
            0,
            taiyin::dispatch::FRAME_ROUTE_EQUINOX,
            0.0,
            0.0,
            0.0,
            0.0,
    ...
    // and date-frame model conventions. With the SOFA-style denominator convention for solar
    ...
    const double jd_utc = jd_tt - 69.184 / taiyin::SECONDS_PER_DAY;
    taiyin::internal::EarthOrientationSample eop_samples[] = {
        { jd_utc - 1.0, -0.0088, 0.086 * taiyin::TAIYIN_ARCSEC_TO_RAD, 0.323 * taiyin::TAIYIN_ARCSEC_TO_RAD, 0.0, 0.0, 0.170 * taiyin::TAIYIN_ARCSEC_TO_RAD / 1000.0, 0.290 * taiyin::TAIYIN_ARCSEC_TO_RAD / 1000.0 },
        { jd_utc + 1.0, -0.0084, 0.088 * taiyin::TAIYIN_ARCSEC_TO_RAD, 0.321 * taiyin::TAIYIN_ARCSEC_TO_RAD, 0.0, 0.0, 0.172 * taiyin::TAIYIN_ARCSEC_TO_RAD / 1000.0, 0.292 * taiyin::TAIYIN_ARCSEC_TO_RAD / 1000.0 },
    };
    eop_samples[0].sp_rad = taiyin::internal::sp_rad_for_jd(eop_samples[0].jd_utc);
    eop_samples[1].sp_rad = taiyin::internal::sp_rad_for_jd(eop_samples[1].jd_utc);
    const taiyin::internal::EarthOrientationTable eop_table = { eop_samples, sizeof(eop_samples) / sizeof(eop_samples[0]) };
    taiyin::runtime::NativeCalcContext topocentric_context;
    expect_true(
        taiyin::runtime::set_global_earth_orientation_table(&eop_table),
        "install global EOP table for Mars precise topocentric oracle path");
    const taiyin::runtime::NativeObserverLocation beijing = taiyin::runtime::native_observer_location_degrees(116.4074, 39.9042, 0.0);
    expect_true(
        taiyin::runtime::native_context_set_precise_topocentric_observer(
            &topocentric_context, beijing, split_jd(jd_utc), split_jd(jd_tt))
            == taiyin::TAIYIN_STATUS_OK,
        "set precise EOP-backed topocentric observer for Mars oracle path");
    expect_true(
        topocentric_context.fields.has(taiyin::runtime::TAIYIN_NATIVE_FIELD_TOPOCENTRIC_OFFSET),
        "Mars precise topocentric path sets offset field");

    double observer_offset_pos[3];
    double observer_offset_vel[3];
    vector_to_array(topocentric_context.apparent_options.observer_offset.position_au, observer_offset_pos);
    vector_to_array(topocentric_context.apparent_options.observer_offset.velocity_au_per_day, observer_offset_vel);
    double topocentric_apparent_position[3];
    expect_true(
        taiyin::calc_apparent_with_matrix(
            jd_tdb,
            4,
            ...
            taiyin::TAIYIN_APPARENT_LIGHT_TIME
                ...
                | taiyin::TAIYIN_APPARENT_USE_MATRIX
                | taiyin::TAIYIN_APPARENT_VELOCITY
                | taiyin::TAIYIN_APPARENT_TOPOCENTRIC,
            0,
            0,
            0,
            0,
            ...
            0,
            0),
        "flat Mars apparent path accepts precise EOP-backed topocentric offset");
    double topocentric_ra_deg = 0.0;
    double topocentric_dec_deg = 0.0;
    expect_true(
        equatorial_ra_dec_degrees(array_to_vector(topocentric_apparent_position), &topocentric_ra_deg, &topocentric_dec_deg),
        "convert topocentric Mars apparent vector to RA/Dec");
    const double topocentric_shift_arcsec = std::sqrt(
        std::pow(signed_degree_difference(topocentric_ra_deg, apparent_ra_deg) * kArcsecPerDegree, 2.0)
        + std::pow((topocentric_dec_deg - apparent_dec_deg) * kArcsecPerDegree, 2.0));
    expect_true(topocentric_shift_arcsec > 0.1, "precise topocentric Mars path differs from geocenter");
    std::printf("flat Mars precise topocentric shift %.6f arcsec\n", topocentric_shift_arcsec);

    expect_true(
        taiyin::runtime::set_global_earth_orientation_table(nullptr),
        "restore empty global EOP after Mars precise topocentric oracle path");
    destroy_storage_ephemeris_block(&mars_storage);
    destroy_storage_ephemeris_block(&earth_storage);
    destroy_storage_ephemeris_block(&sun_storage);
}
