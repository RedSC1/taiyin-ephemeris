...
bool file_exists_string(const std::string& path) {
    return !path.empty() && file_exists(path.c_str());
}

std::string external_spk_path(const char* explicit_path, const char* relative_path) {
    if (explicit_path && explicit_path[0] != '\0') {
        return std::string(explicit_path);
    }
    if (!kNasaBspRoot || !relative_path || kNasaBspRoot[0] == '\0') {
        return std::string();
    }

    std::string path(kNasaBspRoot);
...
bool vector3_position_from_block(const taiyin::SplitJulianDate& jd_tdb, const void* data, taiyin::Vector3* out) {
...
bool vector3_velocity_from_block(const taiyin::SplitJulianDate& jd_tdb, const void* data, taiyin::Vector3* out) {
    const taiyin::internal::CompiledEphemerisBlock* block =
        static_cast<const taiyin::internal::CompiledEphemerisBlock*>(data);
...
void check_split_jd_segment_boundary_selection() {
    using taiyin::SplitJulianDate;
    using taiyin::add_seconds_to_split_jd;
    using taiyin::internal::SpkKernel;
    using taiyin::internal::SpkSegment;
    using taiyin::internal::find_spk_segment;

    constexpr double boundary_et_seconds = 4.0e11;
    SpkKernel kernel;
    SpkSegment before_segment;
    before_segment.start_et_seconds = boundary_et_seconds - 100.0;
    before_segment.end_et_seconds = boundary_et_seconds;
    before_segment.target_id = 499;
    before_segment.center_id = 0;
    before_segment.spk_type = 2;
    SpkSegment after_segment = before_segment;
    after_segment.start_et_seconds = boundary_et_seconds;
    after_segment.end_et_seconds = boundary_et_seconds + 100.0;
    kernel.index.segments.push_back(before_segment);
    kernel.index.segments.push_back(after_segment);

    SplitJulianDate boundary;
    expect_true(
        add_seconds_to_split_jd(
            taiyin::SPLIT_JD_J2000, boundary_et_seconds, &boundary),
        "construct distant SPK segment boundary");
    ...
    expect_true(
        ...
        "distant scalar ET loses the split boundary fraction");
    expect_true(
        find_spk_segment(kernel, 499, 0, just_before)
            == &kernel.index.segments[0],
        "split JD selects the segment before a distant boundary");
    expect_true(
        find_spk_segment(kernel, 499, 0, just_after)
            == &kernel.index.segments[1],
...
const taiyin::internal::EphemerisBlockDescriptor* find_descriptor_covering(
...
void expect_spk_descriptor_covering(
    const std::vector<taiyin::internal::EphemerisBlockDescriptor>& descriptors,
    ...
    const taiyin::internal::EphemerisBlockDescriptor* descriptor =
        find_descriptor_covering(descriptors, target_id, center_id, jd_tdb);
    if (!descriptor) {
        std::fprintf(stderr, "missing SPK discovery descriptor: %s target %d center %d jd %.17g\n", label, target_id, center_id, jd_tdb);
        assert(false);
    }
    expect_true(descriptor->method_id == taiyin::internal::SPK_METHOD_ID, label);
    expect_true(descriptor->format == taiyin::internal::EphemerisBlockFormat::Spk, label);
    expect_true(descriptor->frame == taiyin::internal::EphemerisFrame::IcrfJ2000Equatorial, label);
    expect_true(descriptor->path == path, label);
}

void expect_spk_catalog_candidate(
    const std::vector<taiyin::internal::EphemerisBlockDescriptor>& descriptors,
    int target_id,
    int center_id,
    double jd_tdb,
    ...
    for (size_t i = 0; i < descriptors.size(); ++i) {
        expect_true(catalog.add(descriptors[i]), "add SPK descriptor to catalog");
    }
    ...
    query.jd_tdb = split_jd(jd_tdb);

    std::vector<taiyin::internal::EphemerisBlockDescriptor> candidates;
    expect_true(
        catalog.find_method_candidates(query, taiyin::internal::SPK_METHOD_ID, &candidates),
        label);
    expect_true(!candidates.empty(), label);
    for (size_t i = 0; i < candidates.size(); ++i) {
        expect_true(candidates[i].target_id == target_id, label);
        expect_true(candidates[i].center_id == center_id, label);
        expect_true(candidates[i].method_id == taiyin::internal::SPK_METHOD_ID, label);
        expect_true(candidates[i].jd_tdb_start <= jd_tdb && jd_tdb < candidates[i].jd_tdb_end, label);
    }
}
...
void check_spk_ephemeris_block_range_gate() {
    ...
    expect_true(eval_compiled_ephemeris_block(split_jd(tt_to_tdb_jd_fast(jd)), &block, &state), "eval inside SPK block range");
    expect_true(!eval_compiled_ephemeris_block(split_jd(tt_to_tdb_jd_fast(jd + 2.0)), &block, &state), "reject outside SPK block range");
...
void check_chiron_type21_reference() {
    ...
    expect_true(calc_spk_state(split_jd(tt_to_tdb_jd_fast(jd)), data, &actual), "eval chiron type 21");
    ...
    expect_true(
        eval_compiled_ephemeris_block_acceleration(split_jd(tt_to_tdb_jd_fast(jd)), &chiron_block, &acceleration),
    ...
    const double h = 1e-3;
    expect_true(
        taiyin::internal::eval_compiled_ephemeris_block_velocity(split_jd(tt_to_tdb_jd_fast(jd) - h), &chiron_block, &velocity_before),
        "chiron type 21 velocity before acceleration check");
    expect_true(
        taiyin::internal::eval_compiled_ephemeris_block_velocity(split_jd(tt_to_tdb_jd_fast(jd) + h), &chiron_block, &velocity_after),
    ...
    expect_less(max_vector_diff(acceleration, finite_difference_acceleration), 1e-10, "chiron type 21 acceleration finite-difference fallback");
    ...
    bytes.clear();
    bytes.shrink_to_fit();
    expect_true(calc_spk_state(split_jd(tt_to_tdb_jd_fast(jd)), source_data, &actual), "eval chiron source range");
...
void check_de441_mars_baked_geometric_oracle() {
    using taiyin::CartesianState;
    using taiyin::internal::CompiledEphemerisBlock;
    using taiyin::internal::StorageEphemerisBlock;
    using taiyin::internal::compile_spk_ephemeris_block_from_file;
    ...
    if (!file_exists(kDe441Path)) {
        std::printf("skipping Mars DE441 baked oracle check; local DE441 is absent\n");
        return;
    }
    ...
    expect_true(eval_compiled_ephemeris_block(split_jd(jd_tdb), &mars_heliocentric_block, &mars_heliocentric), "eval Mars heliocentric DE441 oracle");
    ...
    expect_less(max_position_diff_au(mars_heliocentric, expected_mars_heliocentric), 1e-12, "Mars heliocentric baked position oracle diff au");
    expect_less(max_velocity_diff_au_per_day(mars_heliocentric, expected_mars_heliocentric), 1e-10, "Mars heliocentric baked velocity oracle diff au/day");
    ...
    expect_true(eval_compiled_ephemeris_block(split_jd(jd_tdb), &mars_geocentric_block, &mars_geocentric), "eval Mars geocentric DE441 oracle");
    ...
    expect_less(max_position_diff_au(mars_geocentric, expected_mars_geocentric), 1e-12, "Mars geocentric baked position oracle diff au");
    expect_less(max_velocity_diff_au_per_day(mars_geocentric, expected_mars_geocentric), 1e-10, "Mars geocentric baked velocity oracle diff au/day");
...
void check_de441_mars_horizons_apparent_oracle() {
    ...
    const double jd_tt = 2460310.50080074091;  // 2024-01-01T00:00:00Z plus Delta T used by Horizons fixture.
    ...
    expect_true(eval_compiled_ephemeris_block(split_jd(jd_tdb), &earth_heliocentric, &earth_helio_state), "eval Earth heliocentric state");
    expect_true(eval_compiled_ephemeris_block(split_jd(jd_tdb), &earth_barycentric, &earth_bary_state), "eval Earth barycentric state");
    ...
    expect_true(
        eval_compiled_ephemeris_block_acceleration(split_jd(jd_tdb), &earth_barycentric, &earth_barycentric_acceleration),
    ...
    expect_true(
        taiyin::vondrak2011_precession_matrix(split_jd(jd_tt), &precession),
        "Mars apparent precession succeeds");
    expect_true(
        taiyin::iau2000b_nutation(split_jd(jd_tt), &nutation),
        "Mars apparent nutation succeeds");
    const Matrix3x3 true_equator = taiyin::matrix3x3_multiply(taiyin::nutation_matrix(nutation), precession);
...
void check_satellite_and_cob_paths() {
    ...
    expect_true(calc_spk_state(split_jd(jd_tdb), jupiter_center_to_ssb, &path_state), "eval Jupiter center to SSB");
    expect_true(calc_spk_state(split_jd(jd_tdb), jupiter_center_to_barycenter, &cob_state), "eval Jupiter COB offset");
    expect_true(calc_spk_state(split_jd(jd_tdb), jupiter_barycenter_to_ssb, &bary_state), "eval Jupiter barycenter to SSB");
    ...
    expect_true(calc_spk_state(split_jd(jd_tdb), io_to_jupiter_center, &io_center_path), "eval Io to Jupiter center");
    expect_true(calc_spk_state(split_jd(jd_tdb), io_to_jupiter_barycenter, &io_bary_state), "eval Io to Jupiter barycenter");
...
void check_spk_discovery_catalog_routes() {
    using taiyin::internal::DiscoveryOk;
    ...
    if (!file_exists_string(jupiter_path)) {
        std::printf("skipping SPK discovery/catalog Jupiter check; local jup365 BSP is absent\n");
    } else {
        std::vector<EphemerisBlockDescriptor> descriptors;
        EphemerisDiscoveryOptions options;
        expect_true(
            discover_spk_file(jupiter_path, options, &descriptors) == DiscoveryOk,
            "discover Jupiter satellite SPK descriptors");

        expect_spk_descriptor_covering(descriptors, jupiter_path, 501, 5, jd_tdb, "discover Io/Jupiter barycenter direct");
        expect_spk_descriptor_covering(descriptors, jupiter_path, 505, 5, jd_tdb, "discover numeric satellite 505/Jupiter barycenter direct");
        expect_spk_descriptor_covering(descriptors, jupiter_path, 599, 5, jd_tdb, "discover Jupiter body/Jupiter barycenter COB direct");
        expect_spk_descriptor_covering(descriptors, jupiter_path, 5, 0, jd_tdb, "discover Jupiter barycenter/SSB direct");
        expect_spk_descriptor_covering(descriptors, jupiter_path, 10, 0, jd_tdb, "discover Sun/SSB direct");
        expect_spk_descriptor_covering(descriptors, jupiter_path, 501, 599, jd_tdb, "discover Io/Jupiter body derived");
        expect_spk_descriptor_covering(descriptors, jupiter_path, 505, 599, jd_tdb, "discover numeric satellite 505/Jupiter body derived");
    ...
    const std::string asteroid_path =
        external_spk_path(kMainBeltAsteroidsSpkPath, "asteroids/sb441-n16.bsp");
    if (!file_exists_string(asteroid_path)) {
        std::printf("skipping SPK discovery/catalog asteroid check; local sb441-n16 BSP is absent\n");
    } else {
        std::vector<EphemerisBlockDescriptor> descriptors;
        EphemerisDiscoveryOptions options;
        expect_true(
            discover_spk_file(asteroid_path, options, &descriptors) == DiscoveryOk,
            "discover asteroid SPK descriptors");

        expect_spk_descriptor_covering(descriptors, asteroid_path, 2000001, 10, jd_tdb, "discover Ceres heliocentric SPK route");
        expect_spk_descriptor_covering(descriptors, asteroid_path, 2000107, 10, jd_tdb, "discover Camilla heliocentric SPK route");
        expect_spk_descriptor_covering(descriptors, asteroid_path, 2000065, 10, jd_tdb, "discover Cybele heliocentric SPK route");

        expect_spk_catalog_candidate(descriptors, 2000001, 10, jd_tdb, "catalog finds Ceres heliocentric SPK route");
        expect_spk_catalog_candidate(descriptors, 2000107, 10, jd_tdb, "catalog finds Camilla heliocentric SPK route");
    }
}
...
int main() {
    ...
    check_spk_discovery_catalog_routes();
    return 0;
}
