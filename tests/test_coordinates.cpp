...
int main() {
...

    {
        ...
        expect_true(
            taiyin::matrix_derivative_central(
                rotation_z_eval, &data, split_jd(data.epoch_jd), h,
                &matrix_dot),
            "matrix derivative central succeeds",
            &failures);
        expect_true(
            taiyin::matrix_second_derivative_central(
                rotation_z_eval, &data, split_jd(data.epoch_jd), h,
                &matrix_ddot),
        ...
        expect_true(!taiyin::matrix_derivative_central(0, &data, split_jd(data.epoch_jd), h, &matrix_dot), "matrix derivative null eval rejected", &failures);
        expect_true(!taiyin::matrix_second_derivative_central(rotation_z_eval, &data, split_jd(data.epoch_jd), 0.0, &matrix_ddot), "matrix second derivative bad step rejected", &failures);
    }

    {
        ...
        for (int i = 0; i < static_cast<int>(sizeof(oracles) / sizeof(oracles[0])); ++i) {
            ...
            expect_true(taiyin::iau2000b_nutation(split_jd(oracles[i].jd_tt), &actual), "iau2000b succeeds", &failures);
            expect_nutation_near(actual, oracles[i].values, 1e-14, &failures);
            expect_near(
                taiyin::mean_obliquity_iau2006(split_jd(oracles[i].jd_tt)),
                oracles[i].values[2],
                1e-14,
                "mean obliquity oracle",
                &failures);
        }
        expect_true(!taiyin::iau2000b_nutation(split_jd(2451545.0), 0), "iau2000b null out rejected", &failures);
    }

    {
        ...
        expect_true(taiyin::iau2000b_nutation(split_jd(2451545.0), &nutation), "iau2000b for matrix succeeds", &failures);
...

    {
        ...
        for (int i = 0; i < static_cast<int>(sizeof(erfa_oracles) / sizeof(erfa_oracles[0])); ++i) {
            expect_near(
                taiyin::earth_rotation_angle_rad(split_jd(erfa_oracles[i].jd_ut1)),
            ...
            expect_near(
                taiyin::gmst_rad(
                    split_jd(erfa_oracles[i].jd_ut1),
                    split_jd(erfa_oracles[i].jd_tt)),
            ...
            expect_near(
                ...
                taiyin::normalize_radians(
                    taiyin::earth_rotation_angle_rad(
                        split_jd(erfa_oracles[i].jd_ut1))
                    + taiyin::gmst_minus_era_rad(
                        split_jd(erfa_oracles[i].jd_tt))),
                1e-15,
                "GMST composition",
                &failures);
            expect_near(
                taiyin::gmst_minus_era_rad(split_jd(erfa_oracles[i].jd_tt)),
