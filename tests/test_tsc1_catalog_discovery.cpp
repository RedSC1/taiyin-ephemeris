...
void test_non_applicable_and_invalid_files(int* failures) {
    ...
    expect_equal_int(static_cast<int>(discover_tsc1_file("not_a_catalog.bin", options, &descriptors)),
                     static_cast<int>(DiscoveryNotApplicable), "TSC1 ignores opm", failures);
...
void test_tsc1_is_not_builtin_ephemeris_discovery(int* failures) {
    ...
    std::vector<EphemerisBlockDescriptor> descriptors;
    expect_true(
        !discover_ephemeris_descriptors_from_directory(root, discoverers, options, &descriptors),
        "builtin ephemeris discovery ignores TSC1",
        failures);
    expect_equal_size(descriptors.size(), 0, "builtin TSC1 descriptor count", failures);
...
int main() {
    ...
    test_tsc1_is_not_builtin_ephemeris_discovery(&failures);
