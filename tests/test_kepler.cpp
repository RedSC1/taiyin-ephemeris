...
bool eval_compiled_ephemeris_block(
    ...
    return taiyin::internal::eval_compiled_ephemeris_block(split_jd(jd_tdb), block, out);
}
...
bool eval_compiled_ephemeris_block_components(
    ...
    return taiyin::internal::eval_compiled_ephemeris_block_components(
        split_jd(jd_tdb), block, components, out);
}

bool calc_kepler_state(
    double jd_tdb,
    const taiyin::internal::KeplerEphemerisData* data,
    taiyin::CartesianState* out
) {
    return taiyin::internal::calc_kepler_state(split_jd(jd_tdb), data, out);
}
...
bool counted_position(const taiyin::SplitJulianDate&, const void* data, taiyin::Vector3* out) noexcept {
    ComponentEvalCounters* counters = const_cast<ComponentEvalCounters*>(
        static_cast<const ComponentEvalCounters*>(data));
    if (!counters || !out) {
        return false;
    }
    ++counters->position;
...
bool counted_velocity(const taiyin::SplitJulianDate&, const void* data, taiyin::Vector3* out) noexcept {
    ComponentEvalCounters* counters = const_cast<ComponentEvalCounters*>(
        static_cast<const ComponentEvalCounters*>(data));
    if (!counters || !out) {
        return false;
...
bool counted_acceleration(const taiyin::SplitJulianDate&, const void* data, taiyin::Vector3* out) noexcept {
    ComponentEvalCounters* counters = const_cast<ComponentEvalCounters*>(
        static_cast<const ComponentEvalCounters*>(data));
    if (!counters || !out) {
        return false;
...
bool counted_state(const taiyin::SplitJulianDate&, const void* data, taiyin::CartesianState* out) noexcept {
    ComponentEvalCounters* counters = const_cast<ComponentEvalCounters*>(
        static_cast<const ComponentEvalCounters*>(data));
    if (!counters || !out) {
        return false;
    ...
    ++counters->state;
    out->position_au.x = 11.0;
    out->position_au.y = 12.0;
    out->position_au.z = 13.0;
    out->velocity_au_per_day.x = 14.0;
    out->velocity_au_per_day.y = 15.0;
    out->velocity_au_per_day.z = 16.0;
    out->acceleration_au_per_day2.x = 17.0;
    out->acceleration_au_per_day2.y = 18.0;
    out->acceleration_au_per_day2.z = 19.0;
    return true;
}

void check_compiled_block_component_mask() {
    using taiyin::CartesianState;
    using taiyin::internal::CompiledEphemerisBlock;
    using taiyin::internal::EPHEMERIS_BLOCK_COMPONENT_ACCELERATION;
    using taiyin::internal::EPHEMERIS_BLOCK_COMPONENT_POSITION;
    using taiyin::internal::EPHEMERIS_BLOCK_COMPONENT_STATE;
    using taiyin::internal::EPHEMERIS_BLOCK_COMPONENT_VELOCITY;
    using taiyin::internal::eval_compiled_ephemeris_block_components;

    ComponentEvalCounters counters = { 0, 0, 0, 0 };
    CompiledEphemerisBlock block;
    block.data = &counters;
    block.position = counted_position;
    block.velocity = counted_velocity;
    block.acceleration = counted_acceleration;

    CartesianState state;
    expect_true(
        eval_compiled_ephemeris_block_components(split_jd(2451545.0), &block, EPHEMERIS_BLOCK_COMPONENT_POSITION, &state),
        "eval position-only component mask");
    expect_true(counters.position == 1, "position-only calls position");
    expect_true(counters.velocity == 0, "position-only skips velocity");
    expect_true(counters.acceleration == 0, "position-only skips acceleration");
    expect_true(counters.state == 0, "position-only skips state");
    expect_near(state.position_au.x, 1.0, 0.0, "position-only x");
    expect_near(state.velocity_au_per_day.x, 0.0, 0.0, "position-only zero velocity");

    counters = { 0, 0, 0, 0 };
    expect_true(
        eval_compiled_ephemeris_block_components(
            split_jd(2451545.0),
            &block,
            EPHEMERIS_BLOCK_COMPONENT_POSITION | EPHEMERIS_BLOCK_COMPONENT_VELOCITY,
            &state),
        "eval position-velocity component mask");
    expect_true(counters.position == 1, "position-velocity calls position");
    expect_true(counters.velocity == 1, "position-velocity calls velocity");
    expect_true(counters.acceleration == 0, "position-velocity skips acceleration");
    expect_true(counters.state == 0, "position-velocity skips state when unavailable");
    expect_near(state.velocity_au_per_day.x, 4.0, 0.0, "position-velocity x");
    ...
    expect_true(
        eval_compiled_ephemeris_block_components(
            ...
            &state),
        "eval full state component mask");
    expect_true(counters.position == 1, "state calls position");
    expect_true(counters.velocity == 1, "state calls velocity");
    expect_true(counters.acceleration == 1, "state calls acceleration");
    expect_true(counters.state == 0, "state skips state when unavailable");
    expect_near(state.acceleration_au_per_day2.x, 7.0, 0.0, "state acceleration x");

    counters = { 0, 0, 0, 0 };
    block.state = counted_state;
    expect_true(
        eval_compiled_ephemeris_block_components(
            split_jd(2451545.0),
            &block,
            EPHEMERIS_BLOCK_COMPONENT_POSITION | EPHEMERIS_BLOCK_COMPONENT_VELOCITY,
            &state),
        "eval position-velocity with state callback");
    expect_true(counters.position == 0, "position-velocity state callback skips position");
    expect_true(counters.velocity == 0, "position-velocity state callback skips velocity");
    expect_true(counters.acceleration == 0, "position-velocity state callback skips acceleration");
    expect_true(counters.state == 1, "position-velocity calls state callback once");
    expect_near(state.position_au.x, 11.0, 0.0, "state callback position x");
    expect_near(state.velocity_au_per_day.x, 14.0, 0.0, "state callback velocity x");
    expect_near(state.acceleration_au_per_day2.x, 0.0, 0.0, "state callback masks acceleration");
    ...
    expect_true(
        eval_compiled_ephemeris_block_components(
            ...
            EPHEMERIS_BLOCK_COMPONENT_STATE,
            &state),
        "eval full state with state callback");
    expect_true(counters.position == 0, "state callback skips position");
    expect_true(counters.velocity == 0, "state callback skips velocity");
    expect_true(counters.acceleration == 0, "state callback skips acceleration");
    expect_true(counters.state == 1, "state callback called once");
    expect_near(state.acceleration_au_per_day2.x, 17.0, 0.0, "state callback acceleration x");
    ...
    expect_false(
        eval_compiled_ephemeris_block_components(
            ...
            0),
        "reject null output for component mask");
}
...
void check_elliptical_kepler_shape_and_energy() {
    ...
    CartesianState state;
    expect_true(eval_compiled_ephemeris_block(split_jd(2451545.0), &block, &state), "eval periapsis");
    ...
    expect_true(eval_compiled_ephemeris_block(split_jd(2451545.0 + half_period_days), &block, &state), "eval apoapsis");
...
void check_kepler_vs_de441_venus_osculating_drift() {
    ...
    expect_true(eval_compiled_ephemeris_block(split_jd(epoch), &spk_block, &epoch_state), "eval Venus DE441 epoch");
    ...
    for (int i = -4; i <= 4; ++i) {
        ...
        expect_true(eval_compiled_ephemeris_block(split_jd(jd), &spk_block, &spk_state), "eval Venus DE441 sample");
        expect_true(eval_compiled_ephemeris_block(split_jd(jd), &kepler_block, &kepler_state), "eval Venus Kepler sample");
...
int main() {
    ...
    CartesianState state;
    expect_true(eval_compiled_ephemeris_block(split_jd(2451545.0), &block, &state), "eval circular kepler epoch");
    ...
    expect_true(
        eval_compiled_ephemeris_block(split_jd(2451545.0 + quarter_period_days), &block, &state),
    ...
    expect_false(
        eval_compiled_ephemeris_block(split_jd(2451545.0 + 120.0), &block, &state),
    ...
    expect_true(calc_kepler_state(split_jd(106.0), data, &state), "eval second kepler segment");
    expect_false(calc_kepler_state(split_jd(110.5), data, &state), "reject outside segmented runtime range");
    ...
    check_compiled_block_component_mask();
