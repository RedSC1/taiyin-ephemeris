...
taiyin::SplitJulianDate shifted_ut1(
    const taiyin::SplitJulianDate& jd_ut1,
...
// ERFA/SOFA ltpb oracles include the Vondrak et al. (2012) Q_A C7 corrigendum.
const MatrixOracle VONDRAK_ORACLES[] = {
    ...
    { 2460000.0, {
        ...
        { 0.0051765047729521365, 0.99998660179283250, -0.0000057588872958970894 },
        { 0.0022490449222988557, -0.0000058833978765631683, 0.99999747087796709 },
    } },
    { 1219339.078000, {
...
int main() {
    ...
    // IAU2006 precession vs ERFA pfw06+fw2m
    {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            taiyin::internal::erfa_pfw06(
                split_jd(test_jds[i]), &gamb, &phib, &psib, &epsa);
            double erfa_rm[3][3];
            taiyin::internal::erfa_fw2m(gamb, phib, psib, epsa, erfa_rm);

            taiyin::Matrix3x3 ours;
            expect_true(taiyin::iau2006_precession_matrix(split_jd(test_jds[i]), &ours), "iau2006 precession succeeds", &failures);
    ...
    for (int i = 0; i < static_cast<int>(sizeof(CIRS_ORACLES) / sizeof(CIRS_ORACLES[0])); ++i) {
        ...
        expect_true(taiyin::iau2000a_nutation(split_jd(CIRS_ORACLES[i].jd_tt), &nutation), "iau2000a succeeds", &failures);
        ...
        expect_true(taiyin::iau2006a_cip_xy(split_jd(CIRS_ORACLES[i].jd_tt), &cip), "cip xy succeeds", &failures);
        expect_near(cip.x_rad, CIRS_ORACLES[i].x_rad, 2e-18, "cip x", &failures);
        expect_near(cip.y_rad, CIRS_ORACLES[i].y_rad, 4e-17, "cip y", &failures);
        const double s = taiyin::cio_locator_s_iau2006a_rad(split_jd(CIRS_ORACLES[i].jd_tt), cip.x_rad, cip.y_rad);
        expect_near(s, CIRS_ORACLES[i].s_rad, 2e-18, "cio s", &failures);
        taiyin::Matrix3x3 c2i;
        expect_true(taiyin::cirs_matrix_iau2006a(split_jd(CIRS_ORACLES[i].jd_tt), 0.0, 0.0, &c2i), "cirs matrix succeeds", &failures);
        expect_matrix_near(c2i, CIRS_ORACLES[i].c2i, 4e-16, "cirs matrix oracle", &failures);
    }
    ...
    {
        const taiyin::SplitJulianDate jd_tt = split_jd(2460310.5008007409);
        const taiyin::SplitJulianDate jd_ut1 = split_jd(2460310.5);
        const double dut1_rate = 1.3e-4;
        const double lod = 8.0e-4;
        const double lod_rate = -2.0e-5;
        const double h = 0.02;

        const double eq_m2 = taiyin::equation_of_equinoxes_iau2000b_rad(jd_tt - 2.0 * h);
        const double eq_m1 = taiyin::equation_of_equinoxes_iau2000b_rad(jd_tt - h);
        const double eq_0 = taiyin::equation_of_equinoxes_iau2000b_rad(jd_tt);
        const double eq_p1 = taiyin::equation_of_equinoxes_iau2000b_rad(jd_tt + h);
        const double eq_p2 = taiyin::equation_of_equinoxes_iau2000b_rad(jd_tt + 2.0 * h);
        expect_near(
            taiyin::equation_of_equinoxes_rate_iau2000b_rad_per_day(jd_tt, h),
            fourth_order_scalar_rate(eq_m2, eq_m1, eq_p1, eq_p2, h),
            5e-12,
            "equation of equinoxes rate finite difference",
            &failures);
        expect_near(
            taiyin::equation_of_equinoxes_acceleration_iau2000b_rad_per_day2(jd_tt, h),
            fourth_order_scalar_acceleration(eq_m2, eq_m1, eq_0, eq_p1, eq_p2, h),
            1e-12,
            "equation of equinoxes acceleration finite difference",
            &failures);

        const double gast_m2 = taiyin::gast_iau2000b_rad(
            shifted_ut1(jd_ut1, dut1_rate, lod, lod_rate, -2.0 * h),
            jd_tt - 2.0 * h);
        const double gast_m1 = taiyin::gast_iau2000b_rad(
            shifted_ut1(jd_ut1, dut1_rate, lod, lod_rate, -h),
            jd_tt - h);
        const double gast_0 = taiyin::gast_iau2000b_rad(jd_ut1, jd_tt);
        const double gast_p1 = taiyin::gast_iau2000b_rad(
            shifted_ut1(jd_ut1, dut1_rate, lod, lod_rate, h),
            jd_tt + h);
        const double gast_p2 = taiyin::gast_iau2000b_rad(
            shifted_ut1(jd_ut1, dut1_rate, lod, lod_rate, 2.0 * h),
            jd_tt + 2.0 * h);
        expect_near(
            taiyin::gast_rate_iau2000b_rad_per_day(jd_tt, dut1_rate, lod, h),
            fourth_order_angle_rate(gast_m2, gast_m1, gast_0, gast_p1, gast_p2, h),
            1e-7,
            "GAST rate finite difference",
            &failures);
        expect_near(
            taiyin::gast_acceleration_iau2000b_rad_per_day2(jd_tt, lod_rate, h),
