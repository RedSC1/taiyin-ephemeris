...
void test_build_tsc1_bytes_and_provider(int* failures) {
    ...
    expect_true(resolved.runtime_id != 0, "resolved provider runtime id", failures);

    Vector3 position;
    expect_true(
        provider.eval_position(
            "cache-star-alias", taiyin::SplitJulianDate(2451545, 0.0), &position),
        "eval TSF1 provider position",
        failures);
    ...
    expect_true(
        provider.eval_state(
            "cache_star", taiyin::SplitJulianDate(2451546, 0.0), &state),
        "eval TSF1 provider state",
        failures);
...
void test_discovery(int* failures) {
    ...
    expect_true(!discover_ephemeris_descriptors_from_directory(root, discoverers, options, &generic), "builtin ephemeris discovery ignores TSF1", failures);
    bool saw_tsf1 = false;
    ...
    expect_true(!saw_tsf1, "no TSF1 in builtin ephemeris discovery", failures);
