...
int main() {
...

    {
        ...
        for (int i = 0; i < static_cast<int>(sizeof(oracles) / sizeof(oracles[0])); ++i) {
            ...
            expect_true(taiyin::iau2000b_nutation(split_jd(oracles[i].jd_tt), &actual), "IAU2000B oracle succeeds", &failures);
    ...
    {
        // ERFA era00/gmst06 baked fixtures from the baked observer route.
        ...
        for (int i = 0; i < static_cast<int>(sizeof(oracles) / sizeof(oracles[0])); ++i) {
            expect_near(taiyin::earth_rotation_angle_rad(split_jd(oracles[i].jd_ut1)), oracles[i].era_rad, 1e-9, "ERFA ERA oracle", &failures);
            expect_near(taiyin::gmst_rad(split_jd(oracles[i].jd_ut1), split_jd(oracles[i].jd_tt)), oracles[i].gmst_rad, 1e-9, "ERFA GMST oracle", &failures);
            expect_near(
                taiyin::gmst_minus_era_rad(split_jd(oracles[i].jd_tt)),
