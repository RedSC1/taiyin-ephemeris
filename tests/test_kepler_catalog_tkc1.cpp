...
std::string temp_path(const char* suffix) {
    std::string pattern = std::string("/tmp/taiyin-tkc1-XXXXXX") + suffix;
    std::vector<char> templ(pattern.begin(), pattern.end());
    templ.push_back('\0');

    const int suffix_len = static_cast<int>(std::strlen(suffix));
    int fd = mkstemps(&templ[0], suffix_len);
    if (fd >= 0) {
        close(fd);
    }
    return std::string(&templ[0]);
}
...
bool write_file(const std::string& path, const std::vector<uint8_t>& bytes) {
    ...
    const size_t written = std::fwrite(&bytes[0], 1, bytes.size(), file);
    const int close_status = std::fclose(file);
    return written == bytes.size() && close_status == 0;
}
...
void test_load_lookup_and_compile(int* failures) {
    ...
    taiyin::SplitJulianDate eval_jd;
    expect_true(taiyin::split_julian_date_from_double(JD0 + 5.0, &eval_jd), "split eval JD", failures);
    expect_true(eval_compiled_ephemeris_block(eval_jd, &catalog_block, &catalog_state), "eval catalog", failures);
    expect_true(eval_compiled_ephemeris_block(eval_jd, &direct_block, &direct_state), "eval direct", failures);
...
void test_file_and_rejections(int* failures) {
    ...
    const std::vector<uint8_t> bytes = make_fixture();
    expect_true(write_file(path, bytes), "write TKC1 fixture file", failures);
    Tkc1Catalog catalog;
