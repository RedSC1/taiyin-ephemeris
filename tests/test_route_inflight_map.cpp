...
taiyin::internal::EphemerisSegmentCacheKey make_key(int target_id, int64_t item_id) {
    return taiyin::internal::EphemerisSegmentCacheKey(
        taiyin::internal::EphemerisSegmentCacheKindOpm2Segment,
        target_id,
        0,
        10,
        taiyin::internal::IcrfJ2000Equatorial,
        taiyin::internal::EphemerisBlockKey(1, 2, 1, 0),
        item_id);
}
...
void test_same_key_waiters_share_loader(int* failures) {
    ...
    taiyin::internal::EphemerisSegmentCacheKey key = make_key(1, 100);
...
void test_different_keys_load_independently(int* failures) {
    ...
    for (int i = 0; i < 4; ++i) {
        loaders.push_back(std::thread([&, i]() {
            taiyin::internal::EphemerisSegmentCacheKey key = make_key(100 + i, 1);
            while (!start.load()) {
                std::this_thread::yield();
            }
...
void test_failure_wakes_and_allows_retry(int* failures) {
    taiyin::internal::RouteInflightMap inflight;
    taiyin::internal::EphemerisSegmentCacheKey key = make_key(2, 100);
...
void test_guard_cleanup_wakes_waiters(int* failures) {
    taiyin::internal::RouteInflightMap inflight;
    taiyin::internal::EphemerisSegmentCacheKey key = make_key(3, 100);
    std::atomic<bool> waiter_ready(false);
    std::atomic<bool> waiter_finished(false);
